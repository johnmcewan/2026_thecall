...
### names 
mnf_names <- c(
  # Stuart Watson (also Stewart Watson, Bunny Rabbit)
  "stuart watson",
  "stewart watson",
  "bunny rabbit",
  ...
  # Mila Banks (also appears as Mila Lewis Banks in your data)
  "mila banks",
  "mila lewis banks",
  ...
  # Oscar Jamerson (also misspelled Jameson)
  "oscar jamerson",
  "oscar jameson",
  
  # Walt Livingston (also Uncle Walt — hard to compound as a title)
  "walt livingston",
  "uncle walt",
  
  # Isaac Gardner (also Ike Gardner)
  "isaac gardner",
  "ike gardner",
  
  # Jim Nunnelly (also James Nunnelly, Grand Dad Nunnelly)
  "jim nunnelly",
  "james nunnelly",
  "granddad nunnelly",
  "grand dad nunnelly",
  
  # Emanuel Cleaver (also Cleaver alone won't compound — needs first name)
  "emanuel cleaver",
  ...
  # Charles Allen (also Charles Allen III — Roman numerals often drop in plain text)
  "charles allen",
  
  # Harry Sypert (also That Harry — too generic to compound safely)
  "harry sypert",
  ...
  # C. Artis Ashley (initial may tokenize oddly — include both forms)
  "artis ashley",
  "c artis ashley"
)

# plug into your pipeline — after tokens_tolower(), before tokens_remove()
reviews_processed <- tokens_compound(reviews_processed,
                                     pattern = phrase(mnf_names),
...
proper_k <- FindTopicsNumber(reviews_dtm,
                             topics = c(3, 5, 7, 9, 11),  # fewer values to test
                             metrics = c("Griffiths2004", "CaoJuan2009"),  # fewer metrics
                             control = list(seed = 42)
)

####error reached elapsed time limit with this one 
proper_k <- FindTopicsNumber(reviews_dtm,
